...
//		$LNK->SetDebug(__FILE__, __FUNCTION__, __LINE__);	 
	...
	function getRep_pmo9($start, $distr, $sendDate = 0) /*2019_04_01 забрать */
		{		
		...
		$query = 'SELECT * FROM `zhkhform_pasport9`, zhkh_type_data_pasport9,  gis_layers where zhkhform_pasport9.id_type_data  = zhkh_type_data_pasport9.id  AND zhkhform_pasport9.distrId = gis_layers.l_id  		
			...
			ORDER BY  gis_layers.l_id,  sendDate DESC,  zhkhform_pasport9.id_type_data ';		 
//		$LNK->SetDebug(__FILE__, __FUNCTION__, __LINE__);	 
		$LNK->Query($query);
		if($LNK->GetNumRows())
			{		
	...
	function getRep_pmo8($start, $distr, $sendDate = 0) /*2019_04_01 забрать */
		{		
		...
		$query = 'SELECT * FROM `zhkhform_pasport8`, zhkh_type_data_pasport8,  gis_layers where zhkhform_pasport8.id_type_data  = zhkh_type_data_pasport8.id  AND zhkhform_pasport8.distrId = gis_layers.l_id  		
			'.$startAdd.$distrAdd.$sendDateAdd.'		
			ORDER BY  gis_layers.l_id,  sendDate DESC,  zhkhform_pasport8.id_type_data';		 
//		$LNK->SetDebug(__FILE__, __FUNCTION__, __LINE__);	 
		$LNK->Query($query);
		if($LNK->GetNumRows())
			{		
	...
	function getRep_pmo7($start, $distr, $sendDate = 0) /*2019_04_01 забрать */
		...
		$query = 'SELECT * FROM `zhkhform_pasport7`, zhkh_type_data_pasport7,  gis_layers where zhkhform_pasport7.id_type_data  = zhkh_type_data_pasport7.id  AND zhkhform_pasport7.distrId = gis_layers.l_id  		
			'.$startAdd.$distrAdd.$sendDateAdd.'		
			ORDER BY  gis_layers.l_id,  sendDate DESC,  zhkhform_pasport7.id_type_data';		 
//		$LNK->SetDebug(__FILE__, __FUNCTION__, __LINE__);	 
		$LNK->Query($query);
		if($LNK->GetNumRows())
			{		
...
//		$LNK->SetDebug(__FILE__, __FUNCTION__, __LINE__);	 
	...
	function getRep_pmo5($start, $distr, $sendDate = 0) /*2019_04_01 забрать */
		...
		$query = 'SELECT * FROM `zhkhform_pasport5`, zhkh_type_data_pasport5,  gis_layers where zhkhform_pasport5.id_type_data  = zhkh_type_data_pasport5.id  AND zhkhform_pasport5.distrId = gis_layers.l_id  		
			'.$startAdd.$distrAdd.$sendDateAdd.'		
			ORDER BY  gis_layers.l_id,  sendDate DESC,  zhkhform_pasport5.id_type_data';		 
//		$LNK->SetDebug(__FILE__, __FUNCTION__, __LINE__);	 
		$LNK->Query($query);
		if($LNK->GetNumRows())
			{		
	...
	function getRep_pmo4($start, $distr, $sendDate = 0) /*2019_04_01 забрать */
		...
		$query = 'SELECT * FROM `zhkhform_pasport4`, zhkh_type_data_pasport4,  gis_layers where zhkhform_pasport4.id_type_data  = zhkh_type_data_pasport4.id  AND zhkhform_pasport4.distrId = gis_layers.l_id  		
			'.$startAdd.$distrAdd.$sendDateAdd.'		
			ORDER BY  gis_layers.l_id,  sendDate DESC,  zhkhform_pasport4.id_type_data';		 
//		echo $queryAdd ;
//		$LNK->SetDebug(__FILE__, __FUNCTION__, __LINE__);	 
		$LNK->Query($query);
		if($LNK->GetNumRows())
	...
	function getRep_pmo3($start, $distr, $sendDate = 0) /*2019_04_01 забрать */
		...
		$query = 'SELECT * FROM `zhkhform_pasport3`, zhkh_type_data_pasport3,  gis_layers where zhkhform_pasport3.id_type_data  = zhkh_type_data_pasport3.id  AND zhkhform_pasport3.distrId = gis_layers.l_id  		
			'.$startAdd.$distrAdd.$sendDateAdd.'		
			ORDER BY  gis_layers.l_id,  sendDate DESC,  zhkhform_pasport3.id_type_data ';		 
//		echo $queryAdd ;
//		$LNK->SetDebug(__FILE__, __FUNCTION__, __LINE__);	 
		$LNK->Query($query);
		if($LNK->GetNumRows())
...
//		$LNK->SetDebug(__FILE__, __FUNCTION__, __LINE__);	 
	...
	function getRep_pmo1($start, $distr, $sendDate = 0) /*2019_04_01 забрать */
		...
		$query = 'SELECT * FROM `zhkhform_pasport1`, zhkh_type_data_pasport1,  gis_layers where zhkhform_pasport1.id_type_data  = zhkh_type_data_pasport1.id  AND zhkhform_pasport1.distrId = gis_layers.l_id  		
			'.$startAdd.$distrAdd.$sendDateAdd.'		
			ORDER BY  gis_layers.l_id,  sendDate DESC,  zhkhform_pasport1.id_type_data ';		 
//		echo $queryAdd ;
//		$LNK->SetDebug(__FILE__, __FUNCTION__, __LINE__);	 
		$LNK->Query($query);
		if($LNK->GetNumRows())
...
//		$retList = 'zhkh_message.id as messId, userId, user_name, distId, npId, l_name, zhkhSectorId, sphereJkh, orgId, orgName,  orgUsedForce, zhkh_sector.name as zhkhSectorName, initiator, dateCreate, incidentDate, reason, hardwareId, hardware, hardwareType, consumerNum, bossOfWork, bossOfCity, goodNewsMen, tempereture, badNewsMen, status, closeDate 	  ';	
		$retList = 'zhkh_message.id as messId, number, userId, user_name, distId, npId, l_name, zhkhSectorId, sphereJkh, orgId, orgName,  orgUsedForce, zhkh_sector.name as zhkhSectorName, initiator, dateCreate, incidentDate, infrastructureId, infrastructureName, reasonId, reason, hardwareId, hardware, hardwareType, consumerNum, bossOfWork, bossOfCity, goodNewsMen, tempereture, badNewsMen, status, closeDate 	  ';	
		$LNK= new DBLink;
		$query = 'SELECT '.$retList.' FROM `zhkh_message` , users, zhkh_sector, gis_layers  
			where zhkh_message.userId = users.user_id 
			and  zhkh_message.zhkhSectorId = zhkh_sector.id 
...
//		$LNK->SetDebug(__FILE__, __FUNCTION__, __LINE__);	 
	...
	function genMessageNumNext(){ /*2019_04_01 забрать все  оперативные сообщения*/
		$LNK= new DBLink;
		$strYear = date("y");
		$query = 'SELECT *, zhkh_message.id as mes_id  FROM `zhkh_message` 
			ORDER BY mes_id DESC limit 1';		
//		$LNK->SetDebug(__FILE__, __FUNCTION__, __LINE__);	 
		$LNK->Query($query);
		if($LNK->GetNumRows())
			{		
			$msgLast = $LNK->GetData(0, false);
			$numArr = explode('/', $msgLast['number']);
			$ret = ($numArr[0] == date("y")) ? date("y").'/'.($numArr[1]+1) : date("y").'/1';
			}
		else 
			$ret = 0;
		return $ret;
	...
	function _genMessageNum( ){ /*2019_04_01 забрать все  оперативные сообщения*/
		$LNK= new DBLink;	 
		$query = 'SELECT *, zhkh_message.id as mes_id  FROM `zhkh_message` , users, zhkh_sector, gis_layers  
			where zhkh_message.userId = users.user_id 
			and  zhkh_message.zhkhSectorId = zhkh_sector.id 
			and zhkh_message.distId = gis_layers.l_id			
			and zhkh_message.incidentDate < \'2021-01-01 00:00:00\'
			ORDER BY mes_id';		
//		$LNK->SetDebug(__FILE__, __FUNCTION__, __LINE__);	 
		$LNK->Query($query);
		if($LNK->GetNumRows())
			...
			$index = 1;
			foreach ($msgList as $msg) {
					$queryUp =  'UPDATE `zhkh_message` SET  number=\'20\/'.$index.'\' where id  = \''.$msg['mes_id'].'\' ';
					$LNK->Query($queryUp);
					$index ++;
//				$ret[] = (object) $msg;
				}			
...
/******************SETTERS***********************************************************************/	
	function upRep_pmo9($param, $sendDate) /*2019_04_01 добавление нового оперативного сообщения*/
		{
		$LNK= new DBLink;	
//		$LNK->SetDebug(__FILE__, __FUNCTION__, __LINE__);		
		...
		foreach ($param as $name => $val ) {
			if(strpos($name , 'cell_')!== false){
				...
				$value = (isset($tmpArr[3])&&($tmpArr[3] == 't'))? trim($val):floatval($val);
				$row[$tmpArr[2]] .= ', cell_'.$tmpArr[1].' = \''.$value.'\'';
				$counter ++;
			}
		}
		$ret = 0;
		foreach ($row as $name => $val ) {			
			$queryUp = 'UPDATE `zhkhform_pasport9` SET  userId = \''.$param->frm_userId.'\' '.$val.' '.$queryIfAdd.' AND id_type_data = '.$name;
			$LNK->Query($queryUp);
//			echo $queryUp;
			if($LNK->error){
				$ret ++;
...
//		$LNK->SetDebug(__FILE__, __FUNCTION__, __LINE__);	 
	...
	function upRep_pmo8($param, $sendDate) /*2019_04_01 добавление нового оперативного сообщения*/
		{
		$LNK= new DBLink;	
//		$LNK->SetDebug(__FILE__, __FUNCTION__, __LINE__);		
		$counter = $curCell = $rowCnt = 0 ;
		...
		$ret = 0;
		foreach ($row as $name => $val ) {			
			$queryUp = 'UPDATE `zhkhform_pasport8` SET  userId = \''.$param->frm_userId.'\' '.$val.' '.$queryIfAdd.' AND id_type_data = '.$name;
			$LNK->Query($queryUp);
//			echo $queryUp;
			if($LNK->error){
				$ret ++;
...
//		$LNK->SetDebug(__FILE__, __FUNCTION__, __LINE__);	 
	...
	function upRep_pmo7($param, $sendDate) /*2019_04_01 добавление нового оперативного сообщения*/
		{
		$LNK= new DBLink;	
//		$LNK->SetDebug(__FILE__, __FUNCTION__, __LINE__);		
		$counter = $curCell = $rowCnt = 0 ;
		...
		$ret = 0;
		foreach ($row as $name => $val ) {			
			$queryUp = 'UPDATE `zhkhform_pasport7` SET  userId = \''.$param->frm_userId.'\' '.$val.' '.$queryIfAdd.' AND id_type_data = '.$name;
			$LNK->Query($queryUp);
//			echo $queryUp;
			if($LNK->error){
				$ret ++;
...
//		echo $queryAdd;
	...
	function upRep_pmo5($param, $sendDate) /*2019_04_01 добавление нового оперативного сообщения*/
		{
		$LNK= new DBLink;	
//		$LNK->SetDebug(__FILE__, __FUNCTION__, __LINE__);		
		$counter = $curCell = $rowCnt = 0 ;
		...
		$ret = 0;
		foreach ($row as $name => $val ) {			
			$queryUp = 'UPDATE `zhkhform_pasport5` SET  userId = \''.$param->frm_userId.'\' '.$val.' '.$queryIfAdd.' AND id_type_data = '.$name;
			$LNK->Query($queryUp);
//			echo $queryUp;
			if($LNK->error){
				$ret ++;
...
//		$LNK->SetDebug(__FILE__, __FUNCTION__, __LINE__);	 
	...
	function upRep_pmo4($param, $sendDate) /*2019_04_01 добавление нового оперативного сообщения*/
		{
		$LNK= new DBLink;	
//		$LNK->SetDebug(__FILE__, __FUNCTION__, __LINE__);		
		$counter = $curCell = $rowCnt = 0 ;
		...
		$ret = 0;
		foreach ($row as $name => $val ) {			
			$queryUp = 'UPDATE `zhkhform_pasport4` SET  userId = \''.$param->frm_userId.'\' '.$val.' '.$queryIfAdd.' AND id_type_data = '.$name;
			$LNK->Query($queryUp);
//			echo $queryUp;
			if($LNK->error){
				$ret ++;
...
//		$LNK->SetDebug(__FILE__, __FUNCTION__, __LINE__);	 
	...
	function upRep_pmo3($param, $sendDate) /*2019_04_01 добавление нового оперативного сообщения*/
		{
		$LNK= new DBLink;	
//		$LNK->SetDebug(__FILE__, __FUNCTION__, __LINE__);		
		$counter = $curCell = $rowCnt = 0 ;
		...
		$ret = 0;
		foreach ($row as $name => $val ) {			
			$queryUp = 'UPDATE `zhkhform_pasport3` SET  userId = \''.$param->frm_userId.'\' '.$val.' '.$queryIfAdd.' AND id_type_data = '.$name;
			$LNK->Query($queryUp);
//			echo $queryUp;
			if($LNK->error){
				$ret ++;
...
//				$valNew = ()
	...
	function upRep_pmo1($param, $sendDate) /*2019_04_01 добавление нового оперативного сообщения*/
		{
		$LNK= new DBLink;	
//		$LNK->SetDebug(__FILE__, __FUNCTION__, __LINE__);		
		$counter = $curCell = $rowCnt = 0 ;
		...
		$ret = 0;
		foreach ($row as $name => $val ) {			
			$queryUp = 'UPDATE `zhkhform_pasport1` SET  userId = \''.$param->frm_userId.'\' '.$val.' '.$queryIfAdd.' AND id_type_data = '.$name;
			$LNK->Query($queryUp);
			if($LNK->error){
				$ret ++;
				}
...
//		$LNK->SetDebug(__FILE__, __FUNCTION__, __LINE__);	 
	...
	function newMessage($param) /*2019_04_01 добавление нового оперативного сообщения*/
		...
		$queryAdd .= ($param->infrastructureId)?', infrastructureId = \''.trim($param->infrastructureId).'\' ': '';
		$queryAdd .= ($param->infrastructureName)?', infrastructureName = \''.trim($param->infrastructureName).'\' ': '';
		$queryAdd .= ($param->reasonId)?', reasonId = \''.trim($param->reasonId).'\' ': '';
		$queryAdd .= ($param->number)?', number = \''.trim($param->number).'\' ': '';
		$queryAdd .= ($param->reasonName)?', reason = \''.trim($param->reasonName).'\' ': '';
...
//		$LNK->SetDebug(__FILE__, __FUNCTION__, __LINE__);	 
	...
	function deleteFromReport($type, $id){
		$stop = 0;
			switch ($type){
				case 'pmo2': 
					{
					$table = 'zhkhform_pasport2';
				} break;				
				case 'pmo6': 
					{
					$table = 'zhkhform_pasport6';					
				} break;
				default:{
					$stop++;
				};
	...
	//		$LNK->SetDebug(__FILE__, __FUNCTION__, __LINE__);	 
			...
			if($LNK->error)
				...
				$ret['error'] = 1;
				$ret['errorMsg'] = ' Неизвестный тип данных! '.$type;			
		}
		return $ret;			
	}
