	...
	L.Routing.Localization = L.extend(L.Routing.Localization, {
		...
		'ru': {
			directions: {
				...
				SlightRight: 'правее',
				Right: 'направо',
				SharpRight: 'резко вправо',
				SlightLeft: 'левее',
				Left: 'налево',
				SharpLeft: 'резко влево',
				Uturn: 'разворот'
			},
			instructions: {
				// instruction, postfix if the road is named
				'Head':
					['Двигаться {dir}', ' по {road}'],
				'Continue':
					['Продолжать {dir}'],
				'TurnAround':
					['Развернуться'],
				'WaypointReached':
					['Путевая точка достигнута'],
				'Roundabout':
					['Take the {exitStr} exit in the roundabout', ' onto {road}'],
				'DestinationReached':
					['Пункт назначения достигнут'],
				'Fork': ['На развилке, повернуть {modifier}', ' на {road}'],
				'Merge': ['Merge {modifier}', ' onto {road}'],
				'OnRamp': ['Свернуть {modifier} спуск', ' на {road}'],
				'OffRamp': ['Take the ramp on the {modifier}', ' onto {road}'],
				'EndOfRoad': ['Свернуть {modifier} к окончанию дороги', ' по {road}'],
				'Onto': 'по {road}'
			},
			formatOrder: function(n) {
				var i = n % 10 - 1,
